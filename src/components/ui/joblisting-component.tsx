...
export const Supabase = (props: SVGProps<SVGSVGElement>) => (
  ...
  >
    <path
      ...
      fill="url(#paint0_linear_job)"
    />
    <path
      d="M63.7076 110.284C60.8481 113.885 55.0502 111.912 54.9813 107.314L53.9738 40.0627L99.1935 40.0627C107.384 40.0627 111.952 49.5228 106.859 55.9374L63.7076 110.284Z"
      fill="url(#paint1_linear_job)"
      fillOpacity={0.2}
    />
    <path
...
export default function JobListingComponent({
  ...
  const [activeItem, setActiveItem] = React.useState<Job | null>(null)
    ...
    <>
      <AnimatePresence>
        {activeItem ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="pointer-events-none absolute inset-0 z-10 bg-background/40 backdrop-blur-xl dark:bg-background/30"
          />
        ) : null}
      </AnimatePresence>
      <AnimatePresence>
        {activeItem ? (
          <div className="group absolute inset-0 z-10 grid place-items-center p-4">
            <motion.div
              className="flex h-fit w-[90%] max-w-lg cursor-pointer flex-col items-start gap-4 overflow-hidden border border-border bg-background p-4 shadow-lg"
              ref={ref}
              layoutId={`workItem-${activeItem.id ?? activeItem.company}`}
              style={{ borderRadius: 12 }}
            >
              <div className="flex w-full items-center gap-4">
                <motion.div layoutId={`workItemLogo-${activeItem.id ?? activeItem.company}`}>
                  {activeItem.logo}
                </motion.div>
                  ...
                  <div className="flex w-full flex-col gap-0.5">
                    <div className="flex w-full flex-row justify-between gap-0.5">
                      <motion.div
                        ...
                        layoutId={`workItemCompany-${activeItem.id ?? activeItem.company}`}
                      >
                        {activeItem.company}
                      </motion.div>
                    </div>
                    <motion.p
                      layoutId={`workItemTitle-${activeItem.id ?? activeItem.company}`}
                      className="text-sm text-foreground"
                    >
                      {activeItem.title} / {activeItem.salary}
                    </motion.p>
                    <motion.div
                      className="flex flex-row gap-2 text-xs text-foreground"
                      layoutId={`workItemExtras-${activeItem.id ?? activeItem.company}`}
                    >
      ...
      <div className={cn("relative flex items-start", className ?? "")}>
        <div className="relative flex w-full flex-col items-center gap-4 px-2">
          {jobs.map((role) => {
            const itemKey = role.id ?? role.company
            return (
            <motion.div
              layoutId={`workItem-${itemKey}`}
              key={itemKey}
              className="group flex w-full cursor-pointer flex-row items-center gap-4 border border-border bg-background p-2 shadow-sm md:p-4"
              onClick={() => {
                setActiveItem(role)
                onJobClick?.(role)
              }}
            ...
            >
              <motion.div layoutId={`workItemLogo-${itemKey}`}>
                {role.logo}
              </motion.div>
              <div className="flex w-full flex-col items-start justify-between gap-0.5">
                <motion.div
                  className="font-medium text-foreground"
                  layoutId={`workItemCompany-${itemKey}`}
                >
                  {role.company}
                </motion.div>
                <motion.div
                  className="text-xs text-foreground"
                  layoutId={`workItemTitle-${itemKey}`}
                >
                ...
                <motion.div
                  className="flex flex-row gap-2 text-xs text-foreground"
                  layoutId={`workItemExtras-${itemKey}`}
                >
